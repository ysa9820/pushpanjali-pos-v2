...
// EXPANDED ENTERPRISE DATA BLOCKS
const ALL_BLOCKS = [
...
export default function App() {
  ...
  const handleAddSalesman = async () => { await fetch(`http://${serverIP}:5000/api/salesmen`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newSalesman) }); setNewSalesman({ name: '', commissionRate: '' }); fetchData(); };
  ...
  const saveEditedInvoice = async () => {
    ...
    const totalAmt = editingInvoice.items.reduce((s, i) => s + (parseFloat(i.price) * parseInt(i.qty)), 0);
    try {
      const res = await fetch(`http://${serverIP}:5000/api/sales/${editingInvoice.invoice}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cart: editingInvoice.items, totalAmount: totalAmt, paymentMethod: editingInvoice.method, customerName: editingInvoice.customerName, customerMobile: editingInvoice.customerMobile, adminName: 'Master Admin' })
      });
  ...
  // --- DRAG AND DROP & BLOCK PROPERTIES ---
  ...
  const exportSales = () => {
    const rows = [["Invoice No", "Date", "Time", "Customer Name", "Customer Mobile", "Method", "Amount", "Cashier", "Items Sold (Qty)"]];
    sales.forEach(s => rows.push([s.invoice, s.date, s.time, s.customerName||'Walk-in', s.customerMobile||'N/A', s.method, s.amount, s.cashier, s.items.reduce((sum,i)=>sum+parseInt(i.qty),0)]));
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-gray-100 font-sans overflow-hidden relative">
            ...
            <div className="space-y-4">
              ...
              {settings.receiptLayout[editingBlockIndex].id === 'ITEM_TABLE' && (
                <div className="bg-blue-50 p-3 rounded border border-blue-200 mt-4 space-y-2">
                  ...
                  <label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={settings.receiptLayout[editingBlockIndex].props.showSrNo} onChange={e => updateBlockProps('showSrNo', e.target.checked)} /> Show Sr No.</label>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-6">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl flex flex-col h-[85vh]">
            ...
            <div className="p-4 bg-gray-50 border-t flex justify-between items-center rounded-b-xl">
              <div className="font-black text-2xl text-blue-900">New Total: ₹{editingInvoice.items.reduce((s,i)=>s+(i.price*i.qty),0)}</div>
              <button onClick={() => setAppConfirm({show: true, msg: "Save changes? This will instantly adjust the godown stock and customer ledger.", onYes: saveEditedInvoice})} className="bg-green-600 text-white px-8 py-3 rounded-lg font-black text-lg shadow-lg hover:bg-green-700">Apply Edit to Server</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {activeTab === 'DASHBOARD' && (
          <div className="max-w-6xl mx-auto flex flex-col gap-6 mt-4">
            <div className="grid grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-xl shadow-sm border-l-8 border-blue-500"><h3 className="text-gray-500 font-bold text-sm uppercase">Lifetime Sales Revenue</h3><div className="text-4xl font-black text-gray-800 mt-2">₹{totalSalesValuation.toLocaleString('en-IN')}</div></div>
          ...
          <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-sm border overflow-hidden mt-4">
            <div className="bg-gray-100 p-4 border-b flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={exportSales} className="bg-green-600 hover:bg-green-700 text-white font-bold px-4 py-2 rounded shadow">📄 Export Sales (CSV)</button>
              </div>
            </div>
        ...
        {/* --- 3. LEDGERS & BILLS (VOID & EDIT ENGINE) --- */}
        {activeTab === 'LEDGERS' && (
          <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-sm border overflow-hidden mt-4">
            <div className="bg-gray-100 p-4 border-b flex gap-4">
              ...
              <button onClick={() => setSubTab('KHATA')} className={`px-6 py-2 font-bold rounded ${subTab === 'KHATA' ? 'bg-blue-600 text-white shadow-inner' : 'bg-white border text-gray-700 hover:bg-gray-50'}`}>📒 Customer Passbooks</button>
            </div>
            ...
            {subTab === 'KHATA' && (
              <div className="flex h-[600px]">
                ...
                <div className="w-2/3 bg-white p-0 flex flex-col">
                          ...
                          <tbody>
                            {passbookCustomer.history.map((h, i) => (
                              <tr key={i} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-3 font-bold text-xs">{h.type === 'CREDIT_SALE' ? `Bill: ${h.invoice}` : h.type === 'PAYMENT_RECEIVED' ? `Paid via ${h.method}` : h.type}</td>
                  ...
                  ) : (<div className="flex-1 flex items-center justify-center text-gray-400 font-bold">Select a customer from the left to view statement.</div>)}
                </div>
              </div>
            )}
          </div>
        ...
        {/* --- 5. STAFF PERMISSIONS --- */}
        {activeTab === 'STAFF' && (
          <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-sm border p-6 flex flex-col mt-4">
            ...
            <div className="flex gap-4 mb-4 border bg-gray-50 p-4 rounded-lg">
              ...
              <div className="flex flex-col gap-2 w-1/3 border-l pl-4">
                ...
                <label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={newUser.permissions.canDiscount} onChange={e=>setNewUser({...newUser, permissions: {...newUser.permissions, canDiscount: e.target.checked}})} /> Can give discounts (Future Feature)</label>
        ...
        {activeTab === 'FIRM' && (
          <div className="max-w-6xl mx-auto flex gap-6 mt-4">
            <div className="w-1/3 bg-white rounded-xl shadow-sm border p-6 flex flex-col h-[600px]">
