...
// --- 2. START THE EXPRESS SERVER & RAM ENGINE ---
function startServer() {
  ...
  const loadDatabaseToRAM = () => {
    if (!fs.existsSync(dbPath)) {
      dbCache = { 
        ...
        settings: { 
          shopName: "Pushpanjali Fashion Centre", phone: "+91 8767571916", 
          receiptWidth: "110", labelWidth: "50", labelHeight: "25", defaultCommissionRate: 2,
          billShowCustomer: true, billShowSalesperson: true, billFooterMsg: "Thank you for shopping!",
          barcodeShowShopName: true, barcodeShowPrice: true
        }
      };
      atomicSaveToDisk();
    } else {
  ...
  const atomicSaveToDisk = () => {
    try {
      fs.writeFileSync(tempDbPath, JSON.stringify(dbCache, null, 2));
  ...
  serverApp.post('/api/settings', (req, res) => {
    dbCache.settings = { ...dbCache.settings, ...req.body }; atomicSaveToDisk(); res.json({ success: true });
  });

  serverApp.post('/api/login', (req, res) => {
    const user = dbCache.users.find(u => u.pin === req.body.pin);
  ...
  serverApp.post('/api/users', (req, res) => {
    const { id, name, pin, role, code } = req.body;
    if (id) {
      const index = dbCache.users.findIndex(u => u.id === id);
      if (index >= 0) dbCache.users[index] = { id, name, pin, role, code };
    } else {
      dbCache.users.push({ id: Date.now(), name, pin, role, code });
    }
    atomicSaveToDisk(); res.json({ success: true });
  });
  serverApp.delete('/api/users/:id', (req, res) => {
    dbCache.users = dbCache.users.filter(u => u.id !== parseInt(req.params.id)); atomicSaveToDisk(); res.json({ success: true });
  });
  ...
  serverApp.post('/api/customers/pay', (req, res) => {
    ...
    const customer = dbCache.customers.find(c => c.id === customerId);
    if (customer) {
      customer.balance -= Number(amountPaid);
      customer.history.push({ date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString(), type: 'PAYMENT', amount: Number(amountPaid) });
      atomicSaveToDisk(); res.json({ success: true, newBalance: customer.balance });
  ...
  serverApp.delete('/api/inventory/:barcode', (req, res) => {
    dbCache.inventory = dbCache.inventory.filter(item => item.barcode !== req.params.barcode); atomicSaveToDisk(); res.json({ success: true });
  });
  ...
  serverApp.post('/api/checkout', (req, res) => {
    ...
    if (paymentMethod === 'CREDIT') {
      ...
      customer.history.push({ date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString(), invoice: invoiceNo, type: 'CREDIT_SALE', amount: Number(totalAmount) });
    }
    ...
    const saleRecord = {
      ...
      customerName: customerName || 'Walk-in', customerMobile: customerMobile || 'N/A',
      terminal: terminalId, items: cart
    };

    dbCache.sales.push(saleRecord);
    atomicSaveToDisk();
...
app.whenReady().then(() => {
  ...
  const htmlContent = `
    <!DOCTYPE html>
    <html>
      <body style="font-family: sans-serif; background-color: #f3f4f6; color: #1f2937; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; margin: 0;">
        <div style="background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); text-align: center;">
          <h1 style="color: #1e3a8a; margin-top: 0;">🧠 Master Server</h1>
          <div style="background: #d1fae5; color: #047857; padding: 5px 15px; border-radius: 20px; font-weight: bold; display: inline-block; margin-bottom: 20px;">● ONLINE & RUNNING</div>
          <p style="margin: 0; color: #4b5563;">Your POS & Stock Room IP Address is:</p>
          <div style="font-size: 32px; font-weight: bold; font-family: monospace; background: #e5e7eb; padding: 10px; border-radius: 5px; margin: 10px 0; letter-spacing: 2px;">
            ${ipAddress}
          </div>
          <p style="font-size: 12px; color: #9ca3af; max-width: 300px; margin: 20px auto 0;">Leave this window open in the background. Do not close this app.</p>
        </div>
      </body>
    </html>
  `;
